## <center>针对 QWERTY 键位优化的 [NeoVim](https://neovim.io) 配置</center>
...
此配置也提供了“按键→数字→空格”的纵向跳行（位于 `cursor_for_qwerty.vim`），可作为“数字 + 上/下”的替代。

- 向上移动：按 `[`，将 QWERTY 中排（`asdfghjkl;`）视为数字 1→0，依次输入想要的“数字”，再按空格执行。
- 向下移动：按 `'`，其余同上。

例：
| 快捷键                  | 行为                  |
| ----------------------- | --------------------- |
| `[` `a` `;` `;` `SPACE` | 将光标向上移动 100 行 |
| `'` `a` `s` `d` `SPACE` | 将光标向下移动 123 行 |
| `[` `d` `;` `SPACE`     | 将光标向上移动 50 行  |

启用方法：在 `init.lua` 增加：
```
vim.cmd('source ' .. vim.fn.stdpath('config') .. '/cursor_for_qwerty.vim')
```

注意：当前实现最大支持 199 行的跳转。
